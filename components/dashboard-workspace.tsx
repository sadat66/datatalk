...
/** Desktop: overview + embedded chat. Mobile: full-width overview (chat via sidebar → Chat). */
export async function DashboardWorkspace() {
  ...
  return (
    <>
      <div className="hidden min-h-0 flex-1 flex-col lg:flex lg:flex-row">{split}</div>
      <div className="min-h-0 flex-1 overflow-y-auto p-4 sm:p-6 lg:hidden">
        <DashboardOverview data={dashboardData} />
      </div>
    </>
  );
